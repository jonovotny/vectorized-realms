...
Linear cliffs are defined as linestrings. We bevel the start and end of the cliff to avoid sudden right-angled edges. To control the direction of this transition, we can just add two more points to the beginning and end of the ridgeline. Since Turf's offset method requires a polygon as input, we add two more points at each end that can be connected to form a complete polygon without intersecting another outline. These four extra points are removed when creating the detail features.
