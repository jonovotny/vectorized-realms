...
Firefox renders filter effects differently from Webkit browsers like Chrome/Internet Explorer and seems to miss fine details like the dots more easily at various zoom levels. Therefore the visibility of dot features needs to be manually boosted for that browser view (one value in a single filter element, so it is managable):
