 ...
 *  @param {function} options.displayInLayerEditor function that takes a layer and return a boolean if the layer is displayed in the switcher, default test the displayInLayerEditor layer attribute
 ...
 *  @param {boolean} options.collapsed collapse the layereditor at beginning, default true
 ...
 *	- displayInLayerEditor {boolean} display the layer in switcher, default true
 *	- noSwitcherDelete {boolean} to prevent layer deletion (w. trash option = true), default false
 */
var ol_control_LayerEditor = class olcontrolLayerEditor extends ol_control_Control {
  constructor(options) {
    options = options || {}
    var element = ol_ext_element.create('DIV', {
      className: options.switcherClass || 'ol-layereditor'
    })
    super({
      element: element,
    ...
    // displayInLayerEditor
    if (typeof (options.displayInLayerEditor) === 'function') {
      this.displayInLayerEditor = options.displayInLayerEditor
    }
   ...
   */
  displayInLayerEditor(layer) {
    return (layer.get('displayInLayerEditor') !== false)
  }
  /**
  ...
  _setLayerForLI(li, layer) {
    ...
    listeners.push(layer.on('propertychange', (function (e) {
      if (e.key === 'displayInLayerEditor'
        || e.key === 'openInLayerEditor'
        || e.key === 'title'
        || e.key === 'name') {
        this.drawPanel(e)
      }
  ...
  setLayerOpacity(layer, li) {
    var i = li.querySelector('.layereditor-opacity-cursor')
    if (i){
  ...
  */
  ...
  drawList(ul, collection) {
    ...
    function createLi(layer) {
      if (!this.displayInLayerEditor(layer)) {
      ...
      var layer_buttons = ol_ext_element.create('DIV', {
        className: 'ol-layereditor-buttons',
        parent: li
      })
          ...
          keydown: function(e) {
            switch (e.key) {
              ...
              case '+': {
                if (layer.getLayers) {
                  this._focus = layer;
                  layer.set("openInLayerEditor", !layer.get("openInLayerEditor"))
                }
              }
              // Move up dans down
              // fallthrough
      ...
      if (layer.getLayers) {
        ...
        layer.getLayers().forEach(function (l) {
          if (self.displayInLayerEditor(l))
            nb++
        })
        if (nb) {
          ol_ext_element.create('DIV', {
            className: layer.get("openInLayerEditor") ? "collapse-layers" : "expend-layers",
            title: this.tip.plus,
            click: function () {
              var l = self._getLayerForLI(this.parentNode.parentNode)
              l.set("openInLayerEditor", !l.get("openInLayerEditor"))
            },
            parent: layer_buttons
          })
        }
      ...
      if (this.show_progress && layer instanceof ol_layer_Tile) {
        var p = ol_ext_element.create('DIV', {
          className: 'layereditor-progress',
          parent: d
        })
        this.setprogress_(layer)
        layer.layereditor_progress = ol_ext_element.create('DIV', { parent: p })
      }

      // Opacity
      var opacity = ol_ext_element.create('DIV', {
        className: 'layereditor-opacity',
        ...
        click: function (e) {
          ...
          this.parentNode.querySelectorAll('.layereditor-opacity-label')[0].innerHTML = Math.round(op * 100)
        },
        parent: d
      })
      // Start dragging
      ol_ext_element.create('DIV', {
        className: 'layereditor-opacity-cursor ol-noscroll',
      ...
      // Percent
      ol_ext_element.create('DIV', {
        className: 'layereditor-opacity-label',
      ...
      if (layer.getLayers) {
        ...
        if (layer.get("openInLayerEditor") === true) {
  ...
  */
  setprogress_(layer) {
    if (!layer.layereditor_progress) {
      var loaded = 0
      var loading = 0
      var draw = function () {
        if (loading === loaded) {
          loading = loaded = 0
          ol_ext_element.setStyle(layer.layereditor_progress, { width: 0 }) // layer.layereditor_progress.width(0);
        } else {
          ol_ext_element.setStyle(layer.layereditor_progress, { width: (loaded / loading * 100).toFixed(1) + '%' }) // layer.layereditor_progress.css('width', (loaded / loading * 100).toFixed(1) + '%');
        }
      }
...
*/
ol_control_LayerEditor.prototype.tip = {
  up: "up/down",
  down: "down",
  info: "informations...",
...
export default ol_control_LayerEditor
